...
const Status = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // กลุ่ม task ตาม taskId และเลือกเฉพาะงานที่ล่าสุด (createdAt ใหม่สุด)
      const taskMap = {};

      snapshot.docs.forEach(doc => {
        const data = doc.data();
        const taskId = data.taskId;
        const createdAt = data.createdAt?.toDate() || new Date(0); // หากไม่มี createdAt ให้ถือว่าเก่ามาก
        ...
        if (
          ...
          taskMap[taskId] = {
            ...
            createdAtObj: createdAt // เก็บไว้ใช้เปรียบเทียบ
          };
        }
      });
